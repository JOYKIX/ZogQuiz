class RNNoiseProcessor extends AudioWorkletProcessor {
  ...
  process(inputs, outputs) {
    ...
    for (let channel = 0; channel < channels; channel += 1) {
      ...
      const voiceLikely = rms > Math.max(0.0065, this.noiseFloor * (this.mode === "strong" ? 1.55 : 1.42)) && zcr > 0.009 && zcr < 0.26 && !transientLikely;
      const floorRate = voiceLikely ? 0.00016 : 0.009;
      ...
      if (voiceLikely) this.hangoverFrames = 70;
      else this.hangoverFrames = Math.max(0, this.hangoverFrames - 1);

      const openAt = Math.max(0.0068, this.noiseFloor * (this.mode === "strong" ? 1.48 : 1.35));
      const closeAt = Math.max(0.0042, this.noiseFloor * 0.92);
      ...
      if (transientLikely && !inRelease) targetGate *= this.mode === "strong" ? 0.58 : 0.68;
      targetGate = Math.max(this.mode === "strong" ? 0.26 : 0.38, Math.min(1, targetGate));

      const attack = targetGate > this.gateGain ? 0.9 : 0.018;
